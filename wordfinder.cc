...
WordFinder::WordFinder( QObject * parent ):
    ...
    stemmedMinLength( 0 ),
    stemmedMaxSuffixVariation( 0 ),
    inputDicts ( nullptr )
{
    updateResultsTimer.setInterval( 1000 ); // We use a one second update timer
    updateResultsTimer.setSingleShot( true );

    connect( &updateResultsTimer, &QTimer::timeout,
             this, &WordFinder::updateResults, Qt::QueuedConnection );
}
...
void WordFinder::startSearch()
{
    ...
    for( auto & dict : (*inputDicts) )
    {
        vector< wstring > writings = dict->getAlternateWritings( allWordWritings[ 0 ] );

        allWordWritings.insert( allWordWritings.end(), writings.cbegin(), writings.cend() );
    }

    // Query each dictionary for all word writings

    for( const auto & dict : (*inputDicts) )
    {
        if ( ( dict->getFeatures() & requestedFeatures ) != requestedFeatures )
            continue;

        for( const auto & writing : allWordWritings )
        {
            try
            {
                sptr< Dictionary::WordSearchRequest > sr =
                        ( searchType == PrefixMatch ) ?
                            dict->prefixMatch( writing, requestedMaxResults ) :
                            dict->stemmedMatch( writing, stemmedMinLength,
                                                stemmedMaxSuffixVariation,
                                                requestedMaxResults );

                connect( sr.get(), &Dictionary::WordSearchRequest::finished,
                         this, &WordFinder::requestFinished, Qt::QueuedConnection );

                queuedRequests.push_back( sr );
            }
            catch ( std::exception & e )
            {
                qWarning() << QStringLiteral("Word '%1' search error (%2) in '%3'.")
                              .arg(inputWord,e.what(),dict->getName().c_str());
            }
        }
    }
...
void WordFinder::updateResults()
{
    ...
    if ( !resultsArray.empty() )
    {
        if ( searchType == PrefixMatch )
        {
            ...
            for( const auto & wr : allWordWritings )
            {
                wstring target = Folding::applySimpleCaseOnly( wr );
                     ...
                     i != j; ++i )
                {
                    ...
                    else if ( resultNoDia.size() > targetNoDia.size() && resultNoDia.compare( 0, targetNoDia.size(), targetNoDia ) == 0 )
                        rank = PrefixNoDiaMatch * Multiplier + saturated( i->first.size() );
                    else if ( resultNoPunct.size() > targetNoPunct.size() && resultNoPunct.compare( 0, targetNoPunct.size(), targetNoPunct ) == 0 )
                        rank = PrefixNoPunctMatch * Multiplier + saturated( i->first.size() );
                    else if ( resultNoWs.size() > targetNoWs.size() && resultNoWs.compare( 0, targetNoWs.size(), targetNoWs ) == 0 )
        ...
        else
        {
            ...
            for( const auto & wr : allWordWritings )
            {
                wstring target = Folding::apply( wr );

                for( const auto & res : resultsIndex )
                {
                    wstring resultFolded = Folding::apply( res.first );
                    ...
                    if ( res.second->rank > rank )
                        res.second->rank = rank; // We store the best rank of any writing
                }
            }
