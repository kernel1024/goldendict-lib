...
public:
    ...
    DictdDictionary(const DictdDictionary &) = delete;
    DictdDictionary& operator =(DictdDictionary const&) = delete;
    DictdDictionary(DictdDictionary&&) = delete;
    DictdDictionary& operator=(DictdDictionary&&) = delete;
...
string DictdDictionary::getName()
{
                                                             ...
                                                             wstring const & )
{
    try
    {
        ...
        for( auto & alt : alts )
        {
            ...
            vector< WordArticleLink > altChain = findArticles( alt );
        ...
        for( auto & cx : chain )
        {
            ...
            indexFile.seek( cx.articleOffset );
            ...
            char * articleBody = dict_data_read_( dz, articleOffset, articleSize, nullptr, nullptr );
            ...
            wstring headwordStripped =
                    Folding::applySimpleCaseOnly( Utf8::decode( cx.word ) );
        ...
        Dictionary::Initializing & initializing )
{
    ...
    for( const auto & s : fileNames )
    {
        ...
        if ( s.size() < 6 ||
             strcasecmp( s.c_str() + ( s.size() - 6 ), ".index" ) != 0 )
            continue;

        try
        {
            vector< string > dictFiles( 1, s );

            // Check if there is an 'abrv' file present
            string baseName( s, 0, s.size() - 5 );

            dictFiles.emplace_back( string() );
        ...
        {
            qCritical() << "Dictd dictionary reading failed: " << s.c_str() << ". error: " << e.what();
        }
    }
