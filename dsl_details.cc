    ...
    transcriptionCount( 0 )
{
    ...
    Node * textNode = nullptr; // A leaf node which currently accumulates text.
    ...
    if ( !stack.empty() )
        qWarning() << stack.size() << " tags were unclosed.";
}
                           ...
                           bool warn )
{
    ...
    if ( n != stack.rend() )
    {
        ...
        while( !stack.empty() )
        {
            ...
            if ( !found )
                nodesToReopen.emplace_back( Node( Node::Tag(), stack.back()->tagName,
                                               stack.back()->tagAttrs ) );
...
DslIconv::DslIconv( DslEncoding e ):
    Iconv( Iconv::GdWchar, getEncodingNameFor( e ) )
{
}

void DslIconv::reinit( DslEncoding e )
{
